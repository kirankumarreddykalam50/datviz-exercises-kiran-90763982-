...
# -----------------------------------------------------------------------------
# 2. DATA LOADING & PREPROCESSING (FLAT NO INDENTS)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
st.markdown('<div class="main-header">🎮 Global Gaming Industry Analytics Dashboard</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Interactive Exploration of Video Game Sales, Reception Metrics, and Ecosystem Dynamics</div>', unsafe_allow_html=True)
...
# Tabbed Navigation
tab1, tab2, tab3, tab4 = st.tabs(["📊 Market & Sales Trends", "🎮 Ecosystem Dynamics", "🏆 Publisher Performance", "⭐ Perception & Ratings"])

# -----------------------------------------------------------------------------
# TAB 1: Market & Sales Trends
# -----------------------------------------------------------------------------
with tab1:
    st.subheader("Global & Regional Sales Evolution")
    
    yearly_sales = filtered_df.groupby('Year_of_Release')[['NA_Sales', 'EU_Sales', 'JP_Sales', 'Other_Sales']].sum().reset_index()
    fig_line = px.line(
        yearly_sales, 
        x='Year_of_Release', 
        y=['NA_Sales', 'EU_Sales', 'JP_Sales', 'Other_Sales'],
        title="Regional Sales Over Time (Millions of Units)",
        labels={'value': 'Sales (Millions)', 'Year_of_Release': 'Release Year', 'variable': 'Region'},
    ...
    with col_a:
        genre_sales = filtered_df.groupby('Genre')['Global_Sales'].sum().reset_index().sort_values(by='Global_Sales', ascending=False)
        fig_genre = px.bar(
            genre_sales,
            x='Global_Sales',
            y='Genre',
    ...
    with col_b:
        regional_totals = pd.DataFrame({
            ...
            'Sales': [filtered_df['NA_Sales'].sum(), filtered_df['EU_Sales'].sum(), filtered_df['JP_Sales'].sum(), filtered_df['Other_Sales'].sum()]
        })
        fig_pie = px.pie(
            regional_totals,
            names='Region',
...
with tab2:
    ...
    with col_c:
        platform_sales = filtered_df.groupby('Platform')['Global_Sales'].sum().reset_index().sort_values(by='Global_Sales', ascending=False).head(15)
        fig_platform = px.bar(
            platform_sales,
            x='Platform',
            y='Global_Sales',
    ...
    with col_d:
        eco_sales = filtered_df.groupby(['Year_of_Release', 'Ecosystem'])['Global_Sales'].sum().reset_index()
...
with tab3:
    ...
    top_publishers = filtered_df.groupby('Publisher')['Global_Sales'].sum().reset_index().sort_values(by='Global_Sales', ascending=False).head(10)
    fig_pub = px.bar(
        top_publishers,
        x='Publisher',
        y='Global_Sales',
...
# -----------------------------------------------------------------------------
# TAB 4: Perception & Ratings
# -----------------------------------------------------------------------------
with tab4:
    st.subheader("Critic vs. User Ratings Analysis")
    
    scatter_df = filtered_df.dropna(subset=['Critic_Score_Scaled', 'User_Score'])
    ...
    fig_scatter = px.scatter(
        ...
        size='Global_Sales',
        hover_name='Name',
        labels={'Critic_Score_Scaled': 'Critic Score (0-10)', 'User_Score': 'User Score (0-10)'},
        template='plotly_white',
        opacity=0.7
    )
